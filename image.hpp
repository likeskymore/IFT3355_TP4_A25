	...
	void writePNG(std::string const& filename) {
		...
		for (int y = 0; y < height; y++)
			for (int x = 0; x < width; x++) {
				int start = 3 * (y * width + x);
				image[4 * width * y + 4 * x + 0] = (unsigned char)((color[start] - minIntensity) / (maxIntensity - minIntensity) * 255);
				image[4 * width * y + 4 * x + 1] = (unsigned char)((color[start + 1] - minIntensity) / (maxIntensity - minIntensity) * 255);
				image[4 * width * y + 4 * x + 2] = (unsigned char)((color[start + 2] - minIntensity) / (maxIntensity - minIntensity) * 255);
